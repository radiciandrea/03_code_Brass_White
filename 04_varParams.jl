...
function funVarParams(year::Int, site::String, fixedParams)
    ...
    PhP = @. (times.sunset - times.sunrise) / Hour(1)
    ...
    sigma = ifelse.((tas7 .> fixedParams.CTTs) .& (PhP .> fixedParams.CPPs) .& (daySim .< FoA), 0.1, 0.0)# spring hatching rate (1/day) (correction sigma = 0 after august)
    omega = ifelse.((PhP .< CPPa) .& (daySim .> FoJul), 0.5, 0.0) # fraction of eggs going into diapause
    ...
    h = @. (1-fixedParams.epsRat)*(1+fixedParams.eps0)*exp(-fixedParams.epsVar*(prec-fixedParams.epsOpt)^2)/
        (exp(-fixedParams.epsVar*(prec-fixedParams.epsOpt)^2)+ fixedParams.eps0) +
        fixedParams.epsRat*fixedParams.epsDens/(fixedParams.epsDens + exp(-fixedParams.epsFac*H))
    
    # Compute K 
    KR = funKR(prec, daySim, fixedParams.alphaEvap, fixedParams.alphaDens)
